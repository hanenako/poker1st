  ...
  <div class="min-h-screen flex items-center justify-center px-4 py-12">
    <div class="w-full max-w-sm">
      ...
      >
        ...
        <div v-if="route.query.error === 'google'" class="text-sm text-red-600 bg-red-50 rounded-lg px-3 py-2">
        ...
        <p class="text-center text-sm text-zinc-500">
          계정이 없나요?
          <NuxtLink to="/register" class="text-indigo-600 hover:underline">회원가입</NuxtLink>
        </p>
...
async function onSubmit() {
  ...
  try {
    ...
    errorMsg.value = e?.statusMessage || e?.data?.statusMessage || '로그인에 실패했어요.'
  } finally {
    loading.value = false
  }
}
